...
def permutation_invariant_training(
    ...
    **kwargs: Any,
    ...
    # find best
    # if spk_num < 3 or not package_available("scipy"):
    #     if spk_num >= 3 and not package_available("scipy"):
    #         rank_zero_warn(
...
if __name__ == "__main__":
    preds = torch.randn(1, 2, 1)
    target = torch.randn(1, 2, 1)
    print(preds)
    print(target)
    best_metric, best_perm = permutation_invariant_training(
        preds, target, metric_func=torch.nn.functional.mse_loss, mode="speaker-wise", eval_func="min"
    )

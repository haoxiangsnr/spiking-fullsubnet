...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            if mix_y.shape[-1] == self.sample_length:
                rand_start = 0
            else:
                rand_start = np.random.randint(0, mix_y.shape[-1] - self.sample_length)

            mix_y = mix_y[rand_start : rand_start + self.sample_length]
            s1_y = s1_y[rand_start : rand_start + self.sample_length]
            s2_y = s2_y[rand_start : rand_start + self.sample_length]

...
from audiozen.metric import DNSMOS, SISDR, compute_neuronops, compute_synops_v2
...
class Trainer(BaseTrainer):
    ...
    def training_step(self, batch, batch_idx):
        ...
        enhanced_y, enhanced_mag, fb_all_layer_outputs, sb_all_layer_outputs = self.model(noisy_y)
        ...
        synops = compute_synops_v2(
            fb_all_layer_outputs, sb_all_layer_outputs, shared_weights=True, detach_output=False
        )

        loss_synops = 1e-8 * synops

        loss = loss_freq_mae + loss_mag_mae + loss_sdr_norm + loss_synops
        ...
        return {
            "loss": loss,
            "freq_mae": loss_freq_mae,
            "mag_mae": loss_mag_mae,
            "sdr": loss_sdr,
            "synops": synops,
            "sdr_norm": loss_sdr_norm,

...
from efficient_spiking_neuron_return_out import LSTMState, efficient_spiking_neuron
...
class SequenceModel(nn.Module):
    def __init__(
        ...
        if sequence_model == "LSTM":
            ...
            self.sequence_model = S4Model(d_input=input_size, d_output=output_size, d_model=hidden_size,
                                          n_layers=num_layers)
        elif sequence_model == "LIF":
            self.sequence_model = LIFModel(input_size=input_size, hidden_size=hidden_size, output_size=output_size,
                                           )
            bidirectional = False
            # self.sequence_model = nn.GRU(
            #     input_size=input_size,
    ...
    def forward(self, x):
        ...
        if self.sequence_model_name == "LSTM" or self.sequence_model_name == "GRU" or self.sequence_model_name == 'GSU':
        ...
        return o.permute(1, 2, 0).contiguous(), all_layer_outputs  # [T, B, F] => [B, F, T]

...
class Separator(BaseModel):
    ...
    def forward(self, y):
        ...
        df_coefs_list = self.sb_model(noisy_mag, fb_output)
...
if __name__ == "__main__":
    ...
    file_dir = (
        Path("/datasets/datasets_fullband/validation_set/clean").expanduser().absolute()
    )
    clean_files = librosa.util.find_files(file_dir.as_posix(), ext="wav")
    noisy = librosa.load(clean_files[0], sr=16000)[0]
    ...
    config = toml.load(
        ...
        "recipes/intel_ndns/metric_fsb/baseline_onlyGen_freq_MAE_mag_MAE_multiframe.toml"
    )

    model = Separator(**config["model_g"]["args"])
    noisy_y = torch.from_numpy(noisy).unsqueeze(0)
    print(
        model(noisy_y)[0].shape, model(noisy_y)[1].shape
    )  # [1, 1, 16000] [1, 257, 63]
    summary(model, input_data=(noisy_y,), device="cpu")

    # torch.Size([1, 3, 20, 126, 2])
# torch.Size([1, 1, 60, 126, 2])
# torch.Size([1, 1, 176, 126, 2])

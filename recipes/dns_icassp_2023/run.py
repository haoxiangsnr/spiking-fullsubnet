...
def run(config, resume):
    ...
    valid_dataset_dns_1 = instantiate(
        config["validation_dataset_dns_1"]["path"],
        args=config["validation_dataset_dns_1"]["args"],
    )
    valid_dataloader_dns_1 = DataLoader(
        dataset=valid_dataset_dns_1,
        num_workers=0,
        batch_size=1,
    )

    valid_dataset_dns_4_non_personalized = instantiate(
        config["validation_dataset_dns_4_non_personalized"]["path"],
        args=config["validation_dataset_dns_4_non_personalized"]["args"],
    )
    valid_dataloader_dns_4_non_personalized = DataLoader(
        dataset=valid_dataset_dns_4_non_personalized,
        num_workers=0,
        batch_size=1,
    )

    test_dataset_dns_4_non_personalized = instantiate(
        config["test_dataset_dns_4_non_personalized"]["path"],
        args=config["test_dataset_dns_4_non_personalized"]["args"],
    )
    test_dataloader_dns_4_non_personalized = DataLoader(
        dataset=test_dataset_dns_4_non_personalized,
        num_workers=0,
        batch_size=1,
    )
    ...
    for flag in args.mode:
        if flag == "train":
            trainer.train(
                train_dataloader,
                [valid_dataloader_dns_1, valid_dataloader_dns_4_non_personalized],
            )
        elif flag == "validate":
            trainer.validate(
                [valid_dataloader_dns_1, valid_dataloader_dns_4_non_personalized]
            ...
            trainer.test(
                test_dataloader_dns_4_non_personalized, config["meta"]["ckpt_path"]
            )
        elif flag == "predict":
...
if __name__ == "__main__":
    ...
    # 1. extract config path and load config
    # 2. add config path and exp_id to args
